...
type UseButtonStatusProps = {
  onSubmit?: () => void | Promise<void>
  resolveTo?: "success" | "error"
}

const useButtonStatus = ({ onSubmit, resolveTo = "success" }: UseButtonStatusProps) => {
...
const SlideButton = forwardRef<HTMLButtonElement, SlideButtonProps>(
  ({ className, onSlideComplete, resolveTo = "success", ...props }, ref) => {
    ...
    const { status, handleSubmit } = useButtonStatus({ 
      onSubmit: onSlideComplete, 
      resolveTo 
    })

    const dragX = useMotionValue(0)
    const springX = useSpring(dragX, ANIMATION_CONFIG.spring as any)
    ...
    return (
      <motion.div
        ...
        className="shadow-button-inset dark:shadow-button-inset-dark relative flex h-9 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800"
      >
        {!completed && (
          <motion.div
            style={{
            ...
            className="absolute inset-y-0 left-0 z-0 rounded-full bg-green-500 dark:bg-green-600"
          />
        )}
          ...
          {!completed && (
            ...
            >
              <Button
                ...
                size="icon"
                className={cn(
                  "shadow-button rounded-full drop-shadow-xl bg-green-600 hover:bg-green-700",
