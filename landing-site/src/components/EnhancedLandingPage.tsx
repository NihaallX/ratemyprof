...
// API calls will go through Vite proxy to backend on port 8000
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        ...
        const [profsResponse, collegesResponse] = await Promise.all([
          fetch('/api/professors?limit=1'),
          fetch('/api/colleges?limit=1'),
        ]);
        ...
        if (collegesData.colleges && collegesData.colleges.length > 0) {
          ...
          const collegeDetailResponse = await fetch(`/api/colleges/${collegeId}`);
    ...
    const fetchProfessors = async () => {
      try {
        ...
        const response = await fetch('/api/professors?limit=200');
        if (!response.ok) {
  ...
  const handleAuthRedirect = () => {
    ...
    setTimeout(() => {
      if (!authPageLoaded) {
        ...
        const authUrl = getMainAppUrl(`/auth/${authMode === 'signin' ? 'login' : 'signup'}`);
        ...
        <div className="relative z-10 text-center px-4 max-w-5xl mx-auto">
          ...
          >
            ...
            <motion.button
              ...
              onClick={() => {
                sessionStorage.setItem('from_landing', 'true');
                window.location.href = getMainAppUrl('/auth/login');
              }}
      ...
      <footer className="bg-black text-gray-400 border-t border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="flex flex-wrap justify-center items-center gap-x-6 gap-y-3 text-sm mb-4">
            <a href={getMainAppUrl('/help')} className="hover:text-white transition-colors">
              Help
            </a>
            <a href={getMainAppUrl('/guidelines')} className="hover:text-white transition-colors">
              Site Guidelines
            </a>
            <a href={getMainAppUrl('/terms')} className="hover:text-white transition-colors">
              Terms & Conditions
            </a>
            <a href={getMainAppUrl('/privacy')} className="hover:text-white transition-colors">
              Privacy Policy
            </a>
            <a href={getMainAppUrl('/copyright')} className="hover:text-white transition-colors">
              Copyright Compliance
            </a>
            <a href={getMainAppUrl('/data-collection')} className="hover:text-white transition-colors">
              Data Collection Notice
            </a>
            <a href={getMainAppUrl('/contact')} className="hover:text-white transition-colors">
              Contact Us
            </a>
            <a href={getMainAppUrl('/about')} className="hover:text-white transition-colors">
              About Us
            </a>
          </div>
